...
class EduTalkUI:
    ...
    def _process_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE and not self.is_speaking and not self.flashcard_mode:
                    return "stop_recording"
            elif event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # Left click
                    mouse_x, mouse_y = event.pos
                    if (self.window.get_width() // 2 - 50 < mouse_x < self.window.get_width() // 2 + 50) and \
                       (self.window.get_height() // 2 + 150 < mouse_y < self.window.get_height() // 2 + 190):
                        return "quit"
        return None
    ...
    def _render_flashcard_ui(self, center_x, center_y):
        if self.current_flashcard < len(self.assistant.flashcards):
            ...
            text = f"Question: {flashcard['question']}" if not self.show_answer else f"Answer: {flashcard['answer']}"
            self._draw_text(text, center_x, center_y - 50)
            instruction = "Press Space to reveal answer" if not self.show_answer else "Press Space for next"
            self._draw_text(instruction, center_x, center_y + 100, font=self.small_font, color=COLORS["TEXT_SECONDARY"])
        else:
            self.flashcard_mode = False
            self.display_message("No more flashcards.")
            time.sleep(2)
    ...
    def _render_recording_ui(self, center_x, center_y):
        pygame.draw.circle(self.window, COLORS["RECORDING"], (center_x, center_y), UI_CONFIG["REC_SIZE"])
        self._draw_text("Recording... Release Space when done", center_x, center_y + 100, font=self.small_font)

    def _render_speaking_ui(self, center_x, center_y):
        self._draw_text(self.current_display_text, center_x, center_y - 50)
        self._draw_text("Speaking... Press Space to stop", center_x, center_y + 100, font=self.small_font, color=COLORS["TEXT_SECONDARY"])
    ...
    def _draw_text(self, text, center_x, center_y, font=None, color=COLORS["TEXT_PRIMARY"]):
        if font is None:
        ...
        for i, line in enumerate(lines):
            ...
            line_rect = line_surface.get_rect(center=(center_x, start_y + i * line_height))
        ...
        if len(audio_data) > 0:
            ...
            for i in range(UI_CONFIG["WAVEFORM_BARS"]):
                if i * chunk_size < len(audio_data):
                    chunk = audio_data[i*chunk_size:min((i+1)*chunk_size, samples)]
                    amplitude = np.sqrt(np.mean(chunk ** 2)) * UI_CONFIG["WAVEFORM_HEIGHT"] * 3
                    ...
                    x = center_x - (UI_CONFIG["WAVEFORM_BARS"] * bar_width) // 2 + i * bar_width
                    pygame.draw.rect(self.window, color, (x, center_y - bar_height // 2, bar_width - 1, bar_height))
